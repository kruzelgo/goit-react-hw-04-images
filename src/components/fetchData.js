...
//       this.state.images.every(existingImage => existingImage.id !== newImage.id)
//     );

//     return { newImages, totalHits };
//   } catch (error) {
//     console.error('Error fetching images:', error);
//     return { error: true };
//   }
// };
export const fetchData = async (query, page, existingImages) => {
  try {
    const response = await fetch(
      `https://pixabay.com/api/?q=${query}&page=${page}&key=${API_KEY}&image_type=photo&orientation=horizontal&per_page=${perPage}`
    );
    ...
    const newImages = hits.filter(newImage =>
      existingImages.every(existingImage => existingImage.id !== newImage.id)
    );
